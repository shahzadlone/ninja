...
"build/ninja.o: ninja.cc ninja.h eval_env.h manifest_parser.h\n",
      &err));
  ASSERT_EQ("", err);
  EXPECT_EQ("build/ninja.o", std::string(parser_.out_));
...
"  bar.h baz.h\n",
      &err));
  ASSERT_EQ("", err);
  EXPECT_EQ("foo.o", std::string(parser_.out_));
...
"a\\ bc\\ def:   a\\ b c d",
  ...
  EXPECT_EQ("a b",
            std::string(parser_.ins_[0]));
  EXPECT_EQ("c",
            std::string(parser_.ins_[1]));
  EXPECT_EQ("d",
            std::string(parser_.ins_[2]));
}
...
TEST_F(DepfileParserTest, UnifyMultipleOutputs) {
  ...
  ASSERT_EQ("foo", std::string(parser_.out_));
  ASSERT_EQ(3u, parser_.ins_.size());
  EXPECT_EQ("x", std::string(parser_.ins_[0]));
  EXPECT_EQ("y", std::string(parser_.ins_[1]));
  EXPECT_EQ("z", std::string(parser_.ins_[2]));
}
