...
int CLWrapper::Run(const std::string& command, std::string* output) {
  ...
  HANDLE nul =
      CreateFileA("NUL", GENERIC_READ,
                  ...
                  &security_attributes, OPEN_EXISTING, 0, nullptrptr);
  ...
  if (!CreateProcessA(nullptrptr, (char*)command.c_str(), nullptrptr, nullptrptr,
                      /* inherit handles */ TRUE, 0,
                      env_block_, nullptrptr,
  ...
  while (read_len) {
    char buf[64 << 10];
    read_len = 0;
    if (!::ReadFile(stdout_read, buf, sizeof(buf), &read_len, nullptrptr) &&
